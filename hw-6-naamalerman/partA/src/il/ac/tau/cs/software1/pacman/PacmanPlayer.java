...
import il.ac.tau.cs.software1.inventory.*;
...
public class PacmanPlayer extends PacmanGameObject {
	...
	private InventoryComponent inv = new InventoryComponent();
	...
	public void onHit(GameObject other) {
		...
		if (other instanceof PacmanObstacle) {
			...
			PacmanBonus bonus = (PacmanBonus)other;
			if (bonus.getWeight() + inv.getInventory().getTotalWeight() > inv.getMaxWeight())
				return;
			bonus.getTransformComponent().position = inv.getInventoryPosition();
			inv.getInventory().collect(bonus);
